-- All Widgets Theme (Metro)
-- Unified conky theme containing all widgets in a single instance
-- Metro style: flat tiles, bold colors, sharp corners, no borders

-- ###Style###
HTML_color = "#FFFFFF"
HTML_color_border = "#FFFFFF"
transparency_bg = 1.0
transparency_border = 0.0
...
-- ###Metro tile colors (multi-color)###
HTML_color_network = "#0078D4"
HTML_color_bandwidth = "#008272"
HTML_color_processes = "#D83B01"
HTML_color_docker = "#008575"
HTML_color_k8s = "#5C2D91"
HTML_color_crypto = "#E81123"
HTML_color_kev = "#D13438"
HTML_color_infra = "#CA8A04"
HTML_color_weather = "#00B7C3"
HTML_color_calendar = "#107C10"
HTML_color_revisited = "#B146C2"
...
-- Metro per-widget tile colors
r_network, g_network, b_network = hex2rgb(HTML_color_network)
r_bandwidth, g_bandwidth, b_bandwidth = hex2rgb(HTML_color_bandwidth)
r_processes, g_processes, b_processes = hex2rgb(HTML_color_processes)
r_docker, g_docker, b_docker = hex2rgb(HTML_color_docker)
r_k8s, g_k8s, b_k8s = hex2rgb(HTML_color_k8s)
r_crypto, g_crypto, b_crypto = hex2rgb(HTML_color_crypto)
r_kev, g_kev, b_kev = hex2rgb(HTML_color_kev)
r_infra, g_infra, b_infra = hex2rgb(HTML_color_infra)
r_weather, g_weather, b_weather = hex2rgb(HTML_color_weather)
r_calendar, g_calendar, b_calendar = hex2rgb(HTML_color_calendar)
r_revisited, g_revisited, b_revisited = hex2rgb(HTML_color_revisited)

-- Current widget color (set before each widget draw)
r_widget, g_widget, b_widget = r, g, b

function set_widget_color(rw, gw, bw)
    r_widget, g_widget, b_widget = rw, gw, bw
end
...
function draw_square(cr, pos_x, pos_y, rectangle_x, rectangle_y, trans)
    ...
    cairo_set_source_rgba(cr, r_widget, g_widget, b_widget, trans or transparency_bg)
...
function draw_function(cr)
    ...
    if enabled_network then
        set_widget_color(r_network, g_network, b_network)
    ...
    if enabled_bandwidth then
        set_widget_color(r_bandwidth, g_bandwidth, b_bandwidth)
    ...
    if enabled_processes then
        set_widget_color(r_processes, g_processes, b_processes)
    ...
    if enabled_docker then
        set_widget_color(r_docker, g_docker, b_docker)
    ...
    -- K8S
    if enabled_k8s then
        set_widget_color(r_k8s, g_k8s, b_k8s)
    ...
    if enabled_crypto then
        set_widget_color(r_crypto, g_crypto, b_crypto)
    ...
    -- KEV
    if enabled_kev then
        set_widget_color(r_kev, g_kev, b_kev)
        local kx2 = positions["kev-conky-manager"].x
        local ky2 = positions["kev-conky-manager"].y
        draw_vuln_widget(cr, kx2, ky2, "KEV", os.getenv("HOME") .. "/.config/conky/all-widgets-metro-conky-manager/fetch_vulns.py", r_crit_kev, g_crit_kev, b_crit_kev)
    end

    -- Infra
    if enabled_infra then
        set_widget_color(r_infra, g_infra, b_infra)
        local ix = positions["infra-conky-manager"].x
        local iy = positions["infra-conky-manager"].y
        draw_vuln_widget(cr, ix, iy, "INFRA", os.getenv("HOME") .. "/.config/conky/all-widgets-metro-conky-manager/fetch_infra_vulns.py", r_crit_infra, g_crit_infra, b_crit_infra)
    end

    -- Weather
    if enabled_weather then
        set_widget_color(r_weather, g_weather, b_weather)
    ...
    if enabled_calendar then
        set_widget_color(r_calendar, g_calendar, b_calendar)
    ...
    if enabled_revisited then
        set_widget_color(r_revisited, g_revisited, b_revisited)
...
function conky_start_widgets()
    local function draw_conky_function(cr)
        ...
        if not ok then
            local f = io.open(os.getenv("HOME") .. "/.config/conky/all-widgets-metro-conky-manager/error.log", "a")
            if f then
